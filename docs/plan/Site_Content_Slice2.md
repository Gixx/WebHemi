...
> **Status:** implemented (read + write). Copy-paste and media reparent deferred.  
> **Parent:** [Site_Content_Model.md](./Site_Content_Model.md) · [Site_Content_Slice1.md](./Site_Content_Slice1.md) · [CURRENT.md](./CURRENT.md) Phase 10.  
...
| Copy-paste | Defer — disable Copy / paste-copy (no server copy) |
| Cut / DnD move | `PATCH /nodes/{id}` `{ parentId }` for **nodes only** |
| Media move | Defer (no media folder PATCH in this slice) |
...
- Wire: New Folder, New Page, Rename, Delete (soft), Delete in trash (purge), Undo restore, Cut+Paste / DnD node move
- Disable Copy and media-asset move
...
- Deep copy-paste, media reparent
